...
/*
*scan8数组中的索引值就是上面表格中Y、U、V、DY、DU、DV对应的位置
*小写字母yuv是大写字母YUV对应像素点相邻的像素（重构像素、参考作用）
*/
...
//scan8和cache配合使用。
//cache可以看成一个表格的话，scan8[]则存储的是宏块信息在cache中的索引值
//16是4x4块大小，3是亮度色度三通道，3是DC系数
static const uint8_t x264_scan8[16*3 + 3] =
{
    //亮度值
    4+ 1*8, 5+ 1*8, 4+ 2*8, 5+ 2*8,   //0-3
    6+ 1*8, 7+ 1*8, 6+ 2*8, 7+ 2*8,   //4-7
    4+ 3*8, 5+ 3*8, 4+ 4*8, 5+ 4*8,   //8-11
    6+ 3*8, 7+ 3*8, 6+ 4*8, 7+ 4*8,   //12-15
    //色度值（Cb）
    4+ 6*8, 5+ 6*8, 4+ 7*8, 5+ 7*8,   //16-19
    6+ 6*8, 7+ 6*8, 6+ 7*8, 7+ 7*8,   //20-23
    4+ 8*8, 5+ 8*8, 4+ 9*8, 5+ 9*8,   //24-27
    6+ 8*8, 7+ 8*8, 6+ 9*8, 7+ 9*8,   //28-31
    //色度值（Cr）
    4+11*8, 5+11*8, 4+12*8, 5+12*8,   //32-35
    6+11*8, 7+11*8, 6+12*8, 7+12*8,   //36-39
    4+13*8, 5+13*8, 4+14*8, 5+14*8,   //40-43
    6+13*8, 7+13*8, 6+14*8, 7+14*8,   //44-47
    //亮度/色度DC系数值
    0+ 0*8, 0+ 5*8, 0+10*8            //48,49,50
};
